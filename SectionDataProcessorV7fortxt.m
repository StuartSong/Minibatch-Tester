...
for Trials = 1:length(Tablenames)
    ...
    First = find(t_Min_Z<t_Max_Z(1));
    t_Min_Z(First) = [];
    LM_Z_Min(First) = [];

    %% Eliminate the last min data if min goes last
    Last = find(t_Min_Z>t_Max_Z(end));
    t_Min_Z(Last) = [];
    LM_Z_Min(Last) = [];
    ...
    t_Min_Znew = [];
    for p = 1:length(t_Max_Z)-2
        minindex = find(t_Min_Z>t_Max_Z(p+1),1);
    ...
    overlapindex = (t_Max_Z(2:end)-t_Min_Z < 2);
    ...
    for Search = 1:length(t_Min_Z)
        if t_Min_Z(Search)+correction < max(frames)
            Window_Min = t_Max_Z(Search):t_Min_Z(Search)+correction;
        else
            t_Min_X(end) = [];
            t_Max_X(end) = [];
            LM_X_Max(end) = [];
            LM_X_Min(end) = [];
%             Window_Min = t_Max_Z(Search):t_Min_Z(Search);
        end
        [LM_X_Min(Search),LM_X_Min_index] = min(LM_X(Window_Min));
        t_Min_X(Search) = Window_Min(LM_X_Min_index);
        
        Window_Max = t_Min_Z(Search):t_Max_Z(Search+1);
